...
- **`GET /api/claims/:id` was an empty HTTP 500 on production (~4.6s, empty body).** Confirmed live against `https://alpha-court.vercel.app`: `/api/keeper/tick` (no Studio read) was 200, `/cases/1` HTML rendered, and both `/api/claims/1` and `/api/claims` died with no JSON. Root cause: the route handler called Next `connection()` (an RSC primitive) and then an unbounded Studio `get_claim`; Vercel Hobby killed the invocation before the existing `catch` could write a body. The handler now skips `connection()`, bounds Redis/Studio reads, and always returns JSON — a 503 `studio_unavailable` with `detail` and `ms` if Studio is slow from Vercel, or the Redis book if one exists. Staking/live poll also fall back to a browser-side Studio `get_claim` so a visitor is not blocked by a silent 500. Remaining platform limit: a Hobby function still cannot wait forever on Studio; the honest response in that case is 503, not an empty 500.
