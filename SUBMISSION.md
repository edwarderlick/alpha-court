...
*Scope note, stated honestly:* this fix covers the crediting functions' own decision inputs, per the review's explicit examples (`bookGet`, `consensus_result`, `stake_*_total`). The keeper's outer tick loop still discovers *which* claim ids to examine via the Redis-backed book (`loadClaims`/`bookAll`), which itself stays fresh via real chain reads elsewhere in the app (`getAllClaims`'s book-empty fallback, `refreshBook` after every state transition) — a claim-discovery/enumeration concern, distinct from the payout-decision-correctness gap this review raised and the adversarial tests above prove closed.
