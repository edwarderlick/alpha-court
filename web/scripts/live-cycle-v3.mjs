...
function save() {
  writeFileSync(join(dir, "live-cycle-v3.json"), JSON.stringify(out, (_, v) => (typeof v === "bigint" ? v.toString() : v), 2));
}

// Deadline is deliberately extreme (threshold far below any real price) so the
// real leader/validator round has an unambiguous, fast decision -- proving
// the deterministic cross-check engages in real consensus without risking a
// genuinely close call that needs an appeal.
...
console.log("resolving verdict (real leader + validator consensus, deterministic cross-check engaged)...");
